...
import { Heart, Trash2, Clock, BookOpen } from "lucide-react";
  ...
  if (selected) {
    return (
      <div className="animate-page-enter pb-20 sm:pb-10">
        <section className="max-w-2xl mx-auto px-6 sm:px-10 pt-6">
          <button
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              {history.length} {history.length === 1 ? "story" : "stories"} in your library
            </p>
          </div>
          {history.length > 0 && (
            <button
              onClick={clearHistory}
              className="text-xs text-muted-foreground hover:text-destructive transition-colors px-3 py-1.5 rounded-lg border border-border"
            >
              Clear all
            </button>
          )}
